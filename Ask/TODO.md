...
- [x] **모바일 반응형 CSS** (clamp 함수로 전체 UI 최적화)
- [x] **Key 기반 버튼 스타일 분리** (네비게이션 vs 질문 선택)
- [x] **모바일 이미지 크기 최적화** (40% 축소)
- [x] **상단 패딩 축소** (모바일에서 67% 감소)
- [x] **마리 페르소나 정의** (mari_persona.py)
- [x] **2단계 AI 구조 설계 문서화** (AI_ARCHITECTURE.md)

### 🔄 현재 진행 중
- [ ] **2단계 AI 백엔드 구현**
  - [ ] EXPERT_PERSONA 추가 (mari_persona.py)
  - [ ] prompt_builder.py 구현
  - [ ] analyzer.py 2단계 분석 로직
  - [ ] app.py 연동

### 📅 다음 단계
- 2단계 AI 백엔드 완성 → 테스트 → 데이터베이스 → 외부 연동 → 배포

### 🧭 리팩토링 & 신규 기능 계획 (2025-02)
1. **레포 정리 및 성능 계측 파이프라인 구축**
   - 불필요한 설치 로그, 로컬 venv, 미사용 자산 정리
   - Streamlit 실행 경로/데이터 경로 표준화, 로그·CSV 저장 구조 개편
   - Vision/텍스트 분석 병렬 처리 후보를 계측할 수 있는 타이밍 로거 추가
2. **UI 모듈화 & 분석 파이프라인 최적화**
   - `app.py`를 페이지·컴포넌트·서비스 계층으로 분리, CSS는 별도 자산에서 로드
   - Streamlit ↔ AI 호출 간 비동기 이벤트 루프를 재사용하여 대기 시간 감소
   - Vision 분석과 프롬프트 생성을 병렬화해 평균 응답 시간을 단축
3. **PDF 다운로드 & 결과 페이지 고도화**
   - 결과 컴포넌트를 카드형 UI로 재작성하고 Vision 하이라이트/신뢰도 지표 표시
   - Jinja 템플릿 기반 PDF 생성 서비스 구축 후 웹·모바일 다운로드 버튼 제공
   - Supabase/Sheets에 분석 결과와 PDF URL을 함께 기록
...
- **현재 진행률**: 20%
- **핵심 작업**:
  1. [x] 2단계 AI 아키텍처 설계 (AI_ARCHITECTURE.md)
  2. [x] 마리 페르소나 정의 (mari_persona.py 기본 버전)
  3. [ ] EXPERT_PERSONA 추가
  4. [ ] prompt_builder.py 구현
  5. [ ] analyzer.py 2단계 분석 로직
  6. [ ] app.py 연동 및 테스트
- **다음 작업**: EXPERT_PERSONA 및 변환 템플릿 추가
